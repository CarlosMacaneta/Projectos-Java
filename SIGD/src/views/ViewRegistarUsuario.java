 ...
 */
public class ViewRegistarUsuario extends JFrame implements ActionListener {
    ...
    private JLabel lblId = new JLabel("Código do gestor: "+ ID);
    ...
    private void init() {
        ...
        lblSenha.setBounds(450, 382, 100, 30);
        ...
        new Propriedades().setForeground(Color.BLACK, lblId, lblNome, jtfNome, jsNome, jftfNuit, jsData, lblEstadoCivil, jsNuit, lblNacionalidade,
            ...
            lblTipoDocumento, lblDocumento, lblProvincia, lblNuit, lblEmail, jsEmail, lblSenha, jsSenha, jtfBairro, jsBairro, jtfNDocumento,
            jsNDocumento, jtfTelefone, jsTelefone, jtfEmail, jtfUsuario, jpfSenha, jcbTipoDocumento, jcbGenero, jcbProvincia, lblPossuiConta, jsPossuiConta);           
        ...
        new  Propriedades().setFont("Calibri", Font.PLAIN, 14, lblId, lblNome, lblDataNascimento, lblGenero, lblBairro, lblUsuario, lblCelular, lblEstadoCivil, lblTipoDocumento,
            lblDocumento, lblNacionalidade, lblProvincia, lblNuit, lblEmail, lblSenha, lblPossuiConta, jcbEstadoCivil, jcbTipoDocumento, jcbGenero, jcbProvincia);           
        ...
        jpRegistar.setBorder(BorderFactory.createTitledBorder(createLineBorder(new Color(45, 82, 124), 2), "Registar gestor", 1, 0, new Font("Calibri", Font.BOLD, 20), Color.BLACK));
        ...
        new Propriedades().add(this, lblNomeSys, lblIcon, lblId, lblNome, lblUsuario, lblSenha, lblDataNascimento, lblTipoDocumento, jcbTipoDocumento, lblDocumento, lblNacionalidade, jtfNacionalidade, jsNacionalidade,
        ...
        new Propriedades().closeWindow(lblClose, lblClose2);
    }
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == btnRegistar){
            ...
            if(!jtfNome.getText().isEmpty()&&!jftfData.getText().isEmpty()&&!jtfNDocumento.getText().isEmpty()&&!jtfNacionalidade.getText().isEmpty()&&!jftfNuit.getText().isEmpty()
                &&!jtfBairro.getText().isEmpty()&&!jtfTelefone.getText().isEmpty()&&!jtfEmail.getText().isEmpty()&&!jtfUsuario.getText().isEmpty()&&!jpfSenha.getPassword().equals("")){
                ...
                if(ValidarDocumentos.validarDocumento(jcbTipoDocumento, jtfNDocumento)&&CampoData.validarData(strData)&&(ValidarDadosUser.validarTelefone(jtfTelefone)&&ValidarDadosUser.validarEmail(jtfEmail)
                    &&ValidarDadosUser.validarUsername(jtfUsuario)&&ValidarDadosUser.validarSenha(jpfSenha))){
                    ...
                    if(userControl.nivelAcesso() == 1){
                        if(new GestorController().create(new Gestor(ID, jtfUsuario.getText(), Criptografia.encriptografar(ValidarDadosUser.returnSenha(jpfSenha)), userControl.nivelAcesso(),
                        ...
                        }else JOptionPane.showMessageDialog(this, "Ocorreu um erro ao registar gestor", "ERRO", ERROR_MESSAGE);
                    }
                    dispose();
                    new ViewLogin().setVisible(true);
