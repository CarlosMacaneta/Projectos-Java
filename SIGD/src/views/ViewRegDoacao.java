 ...
 */
public class ViewRegDoacao extends JFrame implements ActionListener{
    ...
    private JCheckBox checkEdit = new JCheckBox("Alterar dados");
    ...
    private JLabel lblId = new JLabel("Nº. de documento do doador: ");
    ...
    public void View(){ 
        ...
        checkEdit.setBounds(50, 140, 200, 30);
        jpDonativo.setBounds(20, 195, 910, 450);
        jpText.setBounds(440, 220, 450, 300);
        lblId.setBounds(100-50, 50, 200, 30);
        jtfId.setBounds(230, 50, 200, 30);
        ...
        btnPesquisar.setBounds(430, 50, 35, 30);
        btnEnviar.setBounds(660, 420, 175, 30);
        
        jpTopBar.setBackground(new Color(116, 120, 149));
        new Propriedades().setBackground(new Color(45, 82, 124), btnAdd, btnEditar, btnEnviar, btnRemover, btnPesquisar);
        
        new Propriedades().setForeground(Color.BLACK,lblId,lblNome,jtfNome, lblTipoDoc, jcbTipoDocumento, lblNDocumento, jcbGenero, jtfId,
        ...
        new Propriedades().setFont("Calibri", Font.PLAIN, 14, lblId,lblNome,jtfNome, lblTipoDoc, jcbTipoDocumento, lblNDocumento, lblGenero, jcbGenero, checkEdit,
        ...
        new Propriedades().setLineBorder(new Color(45, 82, 124), 1, jtfId, jtfNome, jtfNDocumento, jtfTelefone, jtfMoeda, jtfNomeProd, jtfQtd, jcbTipoDocumento, jcbGenero, scroll, jtfNomeProd, jtfQtd, 
            jcbCategoria, jcbMoedas, btnAdd, btnEditar, btnRemover, btnEnviar, btnPesquisar);
        new Propriedades().setBorder(BorderFactory.createRaisedBevelBorder(), btnAdd, btnEditar, btnEnviar, btnRemover, btnPesquisar);
        ...
        new Propriedades().setFocusable(false, btnAdd, btnEditar, btnEnviar, btnRemover, btnPesquisar);
        
        new Propriedades().setVisible(false, lblClose2);
        
        new Propriedades().add(this, lblId, lblNome, btnPesquisar, lblTipoDoc, lblNDocumento, lblGenero, jcbGenero, lblTelefone, jtfId, jtfNome, jcbTipoDocumento, jtfNDocumento, jtfTelefone, checkEdit, jpDadosDoador,
        ...
        new Propriedades().addActioListener(this, btnAdd, btnEditar, btnRemover, btnEnviar, btnPesquisar);
    ...
    private void cleanAll(){
        lblId.setText("Nº. de documento do doador: ");
        jtfNome.setText(null);
        jtfNDocumento.setText(null);
        jtfTelefone.setText(null);
        clean();
    ...
    private void addRow(){
        DefaultTableModel model = (DefaultTableModel) tabela.getModel();    
        model.setNumRows(0);
        ArrayList<Donativo> don = new Donativo().findDonativos(jtfNDocumento.getText());
        don_ = don;
        if(!don.isEmpty()){
    ...
    private void enableComponents(boolean bln){
        ...
        jtfNDocumento.setEditable(false);
    ...
    private void fill(String str){
        ArrayList<DoadorDAO> doador_ = new DoadorController().donater(str);
        if(!doador_.isEmpty()){
            jtfNome.setText(doador_.get(0).getNome()+" "+doador_.get(0).getApelido());
            jcbTipoDocumento.setSelectedItem(doador_.get(0).getTipoDocumento());
            jtfNDocumento.setText(doador_.get(0).getnDocumento());
            jcbGenero.setSelectedItem(doador_.get(0).getGenero());
            jtfTelefone.setText(doador_.get(0).getTelefone());
        }else cleanDonater();
    }
        ...
        if(ae.getSource() == btnPesquisar){
            ...
            if(!jtfNome.getText().isEmpty()&&!jtfNDocumento.getText().isEmpty()&&!jtfTelefone.getText().isEmpty()){
                if(tabela.getRowCount() > 0){
                    if(checkEdit.isSelected()){
                        JOptionPane.showMessageDialog(this, "O tipo e número de documento\n não serão alterados.");
                        String[] nome = jtfNome.getText().split(" ");
                        new DoadorController().update(jtfNDocumento.getText(), new DoadorDAO(ValidarDadosUser.primeirosNomes(nome), nome[nome.length - 1], (String)
                            jcbTipoDocumento.getSelectedItem(), jtfNDocumento.getText(), (String) jcbGenero.getSelectedItem(), jtfTelefone.getText(), data));
                        JOptionPane.showMessageDialog(this, "Dados do doador fora alterados com sucesso.");
                    }
                    ...
                    Actividades act = new Actividades(id_, 0, Actividades.countProd(prodQtd), 0, 0, Actividades.countMoney(money), "MZN 0",data, logged.get(0).getId());
