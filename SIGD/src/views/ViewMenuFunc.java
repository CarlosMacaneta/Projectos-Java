 ...
 */
public class ViewMenuFunc extends JFrame implements ActionListener {
    
    private ArrayList<Funcionario> func = new FuncController().lista();
    ...
    public void init(){
        ...
        new Propriedades().setBackground(new Color(22, 41, 80), topBar, jpDate, leftBar, middleCont, btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre,
            btnLogOut, btnMeusDados, btnRedefinir, btnSair, config, btnRemoverConta);
        
        new Propriedades().setLineBorder(Color.white,1, btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre, btnLogOut, btnMeusDados, btnRedefinir, btnSair, btnRemoverConta);
        ...
        new Propriedades().setFocusable(false,  btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre, btnLogOut, btnMeusDados, btnRedefinir, btnSair, btnRemoverConta);
        
        lblSys.setFont(new Font("Bell MT", Font.ITALIC|Font.HANGING_BASELINE, 28));
        new Propriedades().setFont("Calibri", Font.PLAIN, 16, lblTime, lblData, btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre, btnRemoverConta,
            btnLogOut, btnMeusDados, btnRedefinir, btnSair);
        
        new Propriedades().setForeground(Color.white, lblSys, lblTime, lblData,btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre, btnLogOut
        ...
        new Propriedades().setOpaque(true, btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre,
            btnMeusDados, btnRedefinir, btnSair, btnLogOut, btnRemoverConta);
        config.setVisible(false);

        new Propriedades().setPreferredSize(new Dimension(200, 40) , btnOrfanatos, btnDoar, btnDoadores, btnStock,
        ...
        new Propriedades().addContainer(leftBar, lblMenu, btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnLogOut);
        new Propriedades().addContainer(config, lblConfig, btnMeusDados, btnRedefinir, btnSobre, btnSair);
        ...
        new Propriedades().addActioListener(this, btnOrfanatos, btnDoar, btnDoadores, btnStock, btnOpcoes, btnSobre,
            btnCloseTab, btnMeusDados, btnRedefinir, btnSair, btnLogOut);
        setVisible(true);
    }
    ...
    private void changePassword(){
        ...
        for(Funcionario f: func){
            if(f.getId() == logged.get(0).getId()){
                if(f.getSenha().equals(Criptografia.encriptografar(String.valueOf(logged.get(0).getId())))){
                    String senha = JOptionPane.showInputDialog(null, "A sua senha deve ser alterada.\n"
                        + "Digite sua nova senha.");
                    if(!senha.isEmpty()){
                        f.setSenha(Criptografia.encriptografar(senha));
                        new FuncController().update(logged.get(0).getId(), f);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == btnOrfanatos){
            ...
            try {
                Desktop.getDesktop().open(new File("src/sobre/sobre.html"));
            ...
            ArrayList<Log> logged = UserLogged.lista();
            String senha = JOptionPane.showInputDialog(null, "Digite sua nova senha.");
            for(Funcionario f: func){
                if(f.getId() == logged.get(0).getId()){
                    if(!senha.isEmpty()){
                        f.setSenha(Criptografia.encriptografar(senha));
